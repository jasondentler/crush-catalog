...
def get_image_type(data):
    """Detect an image type from raw header bytes."""
    if len(data) < 12:
        return 'unknown'
...
def detect_file_type(file_path):
    """Read a file header and detect the supported image type."""
    try:
...
def get_pillow_from_cr3(cr3_path: str) -> Image.Image:
    """Extract the embedded CR3 JPEG preview or postprocessed image."""
...
def get_pillow_from_file(file_path: str) -> Image.Image:
    """Detect file type and return a Pillow image for CR3 or JPEG input."""
...
def display_image_from_file(file_path: str) -> None:
    """Render an image file in the terminal and close it afterward."""
...
def get_cr3_metadata(cr3_path: str) -> dict:
    """Read raw ExifTool metadata for a CR3 or JPEG file."""
    try:
...
def extract_coordinates_and_time(metadata: dict) -> tuple:
    """Parse decimal GPS coordinates and timestamp from raw ExifTool data."""
    # 1. Fetch Timestamp
