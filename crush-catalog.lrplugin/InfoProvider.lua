...
return {
    sectionsForTopOfDialog = function(f, propertyTable)
        propertyTable.testStatus = "<unknown 5>"
        return {{
            title = LOC "$$$/CrushCatalog/InfoProvider/eBirdKey/Title=eBird",
            ...
            f:column{
                ...
                f:static_text{
                    title = LOC "$$$/CrushCatalog/InfoProvider/eBirdKey/Instructions=Enter your eBird API key. It is stored in Lightroom plug-in preferences on this computer.",
                    width_in_chars = 72
                },
                ...
                f:row{
                    ...
                    f:edit_field{
                        ...
                        width_in_chars = 50,
                        immediate = true,
                        tooltip = LOC "$$$/CrushCatalog/InfoProvider/eBirdKey/Placeholder=Paste your eBird API key here."
                    }
                },
                ...
                f:row{f:push_button{
                    ...
                    action = function(button)
                        ...
                        local statusSuccess = LOC "$$$/CrushCatalog/InfoProvider/eBirdTestStatus/Success=Success!"
                        ...
                        LrTasks.startAsyncTask(function()
                            ...
                            elseif info and info.status == 403 then
                                local errorTestStatus = LOC "$$$/CrushCatalog/InfoProvider/eBirdTestStatus/Forbidden=API key test failed"
                                propertyTable.testStatus = errorTestStatus

                                local dialogTitle = LOC "$$$/CrushCatalog/InfoProvider/eBirdTestErrorDialog/Title=eBird rejected your API key"
                                local dialogText = LOC "$$$/CrushCatalog/InfoProvider/eBirdTestErrorDialog/Text=We received a 403 Forbidden response from eBird."
                                LrDialogs.message(dialogTitle, dialogText)
                            else
                                local errorTestStatus = LOC(
                                    "$$$/CrushCatalog/InfoProvider/eBirdTestStatus/Error=Error: ^1", message)
                                propertyTable.testStatus = errorTestStatus
                            end
                        end)
                    end
                    ...
                    width = 300 -- Add this to reserve space
                }}
            }
        }}
    end,
